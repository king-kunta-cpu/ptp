...
var CL_VERSION = '1.8-ZERO-COST-SHOTTOWER';
...
function freeLLMPollinations(prompt, maxTokens){
  ...
  try{
    ...
    var url='https://text.pollinations.ai/'+encodeURIComponent(prompt.substring(0,2000));
    var r=UrlFetchApp.fetch(url, {muteHttpExceptions:true, headers:{'User-Agent':'CedarLoom/'+CL_VERSION}});
    if(r&&r.getResponseCode()===200){
      var text=r.getContentText();
      if(text&&text.length>10) return {text:text.trim(), provider:'pollinations-free'};
    }
