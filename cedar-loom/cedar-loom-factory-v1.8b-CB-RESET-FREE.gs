...
var CL_VERSION = '1.8b-CB-RESET-FREE';
...
function freeLLMPollinations(prompt, maxTokens){
  ...
  try{
    ...
    // Try multiple free endpoints $0 - best tricks
    var endpoints=[
      'https://text.pollinations.ai/'+encodeURIComponent(prompt.substring(0,2000)),
      'https://text.pollinations.ai/'+encodeURIComponent(prompt.substring(0,2000))+'?model=openai',
      'https://text.pollinations.ai/openai?prompt='+encodeURIComponent(prompt.substring(0,2000))
    ];
    for(var ei=0; ei<endpoints.length; ei++){
      ...
      if(r&&r.getResponseCode()===200){
        ...
        if(text&&text.length>10){
          // Check if error about budget - skip
          if(text.toLowerCase().indexOf('budget')>=0 && text.toLowerCase().indexOf('api key')>=0){
            log('WARN','freeLLM','endpoint '+ei+' budget error, trying next');
            continue;
          }
          return {text:text.trim(), provider:'pollinations-free-'+ei};
        }
      }
    }
