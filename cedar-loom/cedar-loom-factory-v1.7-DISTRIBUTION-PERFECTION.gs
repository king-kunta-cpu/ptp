...
var CL_VERSION = '1.7-DISTRIBUTION-PERFECTION';
...
var CL = {
  ...
  DIST_VALUE_RATIO: 80, // 80% value, 20% promo
  DIST_MAX_PER_TICK: 5, // increased from 3 to 5 for loudest noise
...
function getSpreadsheet() {
  ...
  var id = '';
  try { id = PropertiesService.getScriptProperties().getProperty('GW_SHEET_ID') || PropertiesService.getScriptProperties().getProperty('CL_SHEET_ID') || ''; } catch(e){}
  if (id) { try { _ssCache = SpreadsheetApp.openById(id); return _ssCache; } catch(e){ _ssCache=null; } }
  try { var ss = SpreadsheetApp.getActiveSpreadsheet(); if (ss) { _ssCache=ss; return _ssCache; } } catch(e){}
...
function safeFetch(url, options, maxRetries){ maxRetries=Math.min(2,Math.max(1,maxRetries||2)); var last=''; for(var i=1;i<=maxRetries;i++){ try{ var r=UrlFetchApp.fetch(url,options); var code=r.getResponseCode(); if(code===429){ var ra=0; try{ ra=Number(r.getHeaders()['Retry-After']||0); }catch(e){} if(i<maxRetries&&ra<=30){ sleep(((ra>0)?ra:Math.pow(2,i))*1000+Math.floor(Math.random()*400)); last='HTTP 429'; continue; } return r; } if(code>=500&&code<=599){ sleep(Math.pow(2,i)*1000+Math.floor(Math.random()*500)); last='HTTP '+code; continue; } return r; }catch(e){ last=e.toString(); if(i<maxRetries) sleep(Math.pow(2,i)*1000); } } log('ERROR','safeFetch','all '+maxRetries+' failed - '+maskSecret(last)); return null; }
...
function log(level, fn, message){ var safe=''; try{ safe=maskSecret(String(message==null?'':message).replace(/^=+/,' ')); }catch(e){ safe=String(message); } try{ Logger.log('['+level+'] '+fn+' | '+safe); }catch(e2){} _LOG_BUF.push([nowIso(),level,fn,safe,_runId()]); if(level==='ERROR'||level==='WARN'){ flushLogs(); return; } if(_LOG_BUF.length>=_LOG_FLUSH_MAX) flushLogs(); }
...
function enqueueValuePosts(count){
  ...
  try{
    ...
    for(var pi=0; pi<personas.length; pi++){
      ...
      for(var vi=0; vi<count; vi++){
        ...
        appendRow('Distribution',{
          ...
          asset_name:'Value: '+kw,
          persona_id:persona.persona_id,
          channel:['bluesky','mastodon','buffer_x'][vi%3], // rotate channels for loudest noise
          status:D.QUEUED,
          attempts:0,
          next_attempt_ts:Date.now()+ (2+Math.floor(Math.random()*6))*3600000, // 2-8h jitter for value
          post_text:valuePost.text,
          cta_url:'', // value has no CTA or soft CTA only
          remote_id:'',
          remote_url:'',
          error:'',
          created_at:nowIso(),
    ...
    if(queued) log('SUCCESS','enqueueValue',queued+' value posts queued (80% value)');
    return queued;
...
function drainDistributionQueueV2(){
  ...
  Object.keys(postsTodayPromo).forEach(function(pid2){
    ...
    if(p&&Number(p.posts_today||0)!==postsTodayPromo[pid2]){
      try{ updateRow('Personas',p._row,{posts_today:postsTodayPromo[pid2],last_post_utc:nowIso()}); }catch(e){}
    }
  });
  ...
  try{
    ...
    for(var i=0;i<assets.length;i++){
      ...
      if(aid===String(ref)||aname===String(ref)){
        var u=String(assets[i].screenshot_url||assets[i].cdn_url||'').trim();
        if(u && u.indexOf('failed:')===-1) out.push(u);
      }
    }
...
function pollRenders(){
  ...
  for(var i=0;i<rows.length;i++){
    ...
    if(!budgetReserve(20000)) break;
    var res=(String(v.provider||'')==='shotstack')?_pollShotstack(String(v.job_id)):_pollJson2video(String(v.job_id));
    if(res.status==='done'){
...
function drainDistributionQueue(){
  ...
  Object.keys(postsToday).forEach(function(pid2){ var p=personas[pid2]; if(p&&Number(p.posts_today||0)!==postsToday[pid2]){ try{ updateRow('Personas',p._row,{posts_today:postsToday[pid2],last_post_utc:nowIso()}); }catch(e){} } });
...
function clMain(){
  return withLock('clMain', function(){
    ...
    log('INFO','clMain','start mode='+(isHeavy?'WEEKEND-HEAVY':'WEEKDAY-LIGHT')+' budget '+budgetInfo.used+'/'+budgetInfo.budget+'ms left '+(budgetInfo.budget-budgetInfo.used)+'ms');
    ...
    if(isHeavy){
      try{
        ...
        if(ideasQueue<20 && (getConfig('OFFLOAD_ENABLED','TRUE')+'').toUpperCase()==='TRUE'){
          var target=parseInt(getConfig('HEAVY_RESEARCH_TARGET','30'),10)||30;
          dispatchHeavyBuild(target);
        } else if(ideasQueue<20){
          runResearchBatch(target);
        }
      }catch(e){ log('ERROR','researchHeavy',e.toString()); }
      try{ draftAssetBriefsBatch(10); }catch(e){ log('ERROR','draftHeavy',e.toString()); }
      try{ buildPendingAssetsBatch(10); }catch(e){ log('ERROR','buildHeavy',e.toString()); }
      try{ deployAssetLandingsBatch(10); }catch(e){ log('ERROR','landingHeavy',e.toString()); }
      try{ listOnWhop(); }catch(e){ log('ERROR','whop',e.toString()); }
      try{ createBundleIfDue(); }catch(e){ log('ERROR','bundle',e.toString()); }
      try{ enqueueValuePosts(20); }catch(e){ log('ERROR','enqueueValueHeavy',e.toString()); }
      ...
      try{ engageWithPotentialCustomers(); }catch(e){ log('ERROR','engageHeavy',e.toString()); }
    } else {
      try{
        var q=readSheet('Backlog').filter(function(r){ return String(r.status||'').toUpperCase()===Q.PENDING; }).length;
        if(q<5) runResearchBatch(parseInt(getConfig('LIGHT_RESEARCH_TARGET','2'),10)||2);
      ...
      try{ deployAssetLandingsBatch(2); }catch(e){ log('ERROR','landingLight',e.toString()); }
      try{ listOnWhop(); }catch(e){ log('ERROR','whop',e.toString()); }
      try{ enqueueValuePosts(5); }catch(e){ log('ERROR','enqueueValue',e.toString()); }
      try{ enqueueFreeAssetDistribution(); }catch(e){ log('ERROR','enqueue',e.toString()); }
      try{ drainDistributionQueueV2(); }catch(e){ log('ERROR','drainV2',e.toString()); }
      try{ engageWithPotentialCustomers(); }catch(e){ log('ERROR','engage',e.toString()); }
      try{ processCustomRequests(); }catch(e){ log('ERROR','custom',e.toString()); }
      try{
        var top=getTopRevenueAssets(3);
        if(top.length){ log('INFO','pareto','top revenue: '+top.map(function(a){return a.name;}).join(', ')); }
      }catch(e){}
    }
    ...
    log('INFO','clMain','tick done '+(Date.now()-start)+'ms today '+after.used+'ms left '+(after.budget-after.used)+'ms mode='+(after.isHeavy?'HEAVY':'LIGHT'));
  });
}
...
function testLlm(){
  ...
  var text=out.join('\n'); log('INFO','testLlm',text.replace(/\n/g,' | ')); flushLogs(); try{ SpreadsheetApp.getUi().alert('LLM Diagnostic',text,SpreadsheetApp.getUi().ButtonSet.OK); }catch(e){} return text;
}
function testDistribution(){
  ...
  var out=lines.join('\n'); log('INFO','testDistribution',out.replace(/\n/g,' | ')); try{ SpreadsheetApp.getUi().alert('Distribution Check',out,SpreadsheetApp.getUi().ButtonSet.OK); }catch(e){} return out;
}
function testWhop(){
  ...
  var msg=(ok?'✓ Whop configured':'X Fix below')+'\n\n'+steps.join('\n'); log(ok?'SUCCESS':'ERROR','testWhop',steps.join(' | ')); try{ SpreadsheetApp.getUi().alert('Whop Check',msg,SpreadsheetApp.getUi().ButtonSet.OK); }catch(e){} return {ok:ok,steps:steps};
}
function testYouTube(){
  ...
  var videos=[]; try{ videos=readSheet('Videos'); }catch(e){}
  ...
  var text=out.join('\n'); log('INFO','testYouTube',text.replace(/\n/g,' | ')); flushLogs(); try{ SpreadsheetApp.getUi().alert('YouTube Diagnostic',text,SpreadsheetApp.getUi().ButtonSet.OK); }catch(e){} return text;
}
...
function testMastodon(){
  ...
  var msg=lines.join('\n'); log(ok?'SUCCESS':'ERROR','testMastodon',lines.join(' | ')); try{ SpreadsheetApp.getUi().alert('Mastodon Check',msg,SpreadsheetApp.getUi().ButtonSet.OK); }catch(e){} return msg;
}
...
function testBuffer(){
  ...
  var msg=lines.join('\n'); log(ok?'SUCCESS':'ERROR','testBuffer',lines.join(' | ')); try{ SpreadsheetApp.getUi().alert('Buffer Check',msg,SpreadsheetApp.getUi().ButtonSet.OK); }catch(e){} return msg;
}
...
function testBluesky(){
  ...
  var msg=lines.join('\n'); log(ok?'SUCCESS':'ERROR','testBluesky',lines.join(' | ')); try{ SpreadsheetApp.getUi().alert('Bluesky Check',msg,SpreadsheetApp.getUi().ButtonSet.OK); }catch(e){} return msg;
}
function testWhopForum(){
  ...
  var msg=lines.join('\n'); log(ok?'SUCCESS':'ERROR','testWhopForum',lines.join(' | ')); try{ SpreadsheetApp.getUi().alert('Whop Forum Check',msg,SpreadsheetApp.getUi().ButtonSet.OK); }catch(e){} return msg;
}
function reconcile(){
...
function watchdog(){
  var out=[];
  try{
    var last=''; try{ last=PropertiesService.getScriptProperties().getProperty('CL_LAST_RUN')||''; }catch(e){}
...
function trimSheet(name,maxRows){ try{ var sh=getSpreadsheet().getSheetByName(name); if(!sh) return 0; var excess=sh.getLastRow()-maxRows-1; if(excess<=0) return 0; sh.deleteRows(2,excess); return excess; }catch(e){ return 0; } }
function trimAllSheets(){ var t=0; t+=trimSheet('Audit',CL.MAX_AUDIT_ROWS); t+=trimSheet('Revenue',CL.MAX_REVENUE_ROWS); t+=trimSheet('Logs',CL.MAX_LOG_ROWS); try{ var rows=readSheet('Distribution'); if(rows.length>CL.MAX_DIST_ROWS){ var sh=getSpreadsheet().getSheetByName('Distribution'); var removed=0; for(var i=rows.length-1;i>=0&&rows.length-removed>CL.MAX_DIST_ROWS;i--){ var st=String(rows[i].status||''); if(st!==D.POSTED&&st!==D.FAILED&&st!==D.SKIPPED) continue; sh.deleteRow(rows[i]._row); removed++; } t+=removed; } }catch(e){} return t; }
...
function installTriggers(){
  ScriptApp.getProjectTriggers().forEach(function(t){ var h=t.getHandlerFunction(); if(h==='clMain'||h==='watchdog') try{ ScriptApp.deleteTrigger(t); }catch(e){} });
...
function testValuePost(){
  ...
  try{ SpreadsheetApp.getUi().alert('Value Post',msg,SpreadsheetApp.getUi().ButtonSet.OK); }catch(e){}
  return msg;
}
function testBskySearch(){
  ...
  try{ SpreadsheetApp.getUi().alert('Bsky Search',msg,SpreadsheetApp.getUi().ButtonSet.OK); }catch(e){}
  return msg;
}
function testMastodonSearch(){
  ...
  try{ SpreadsheetApp.getUi().alert('Mastodon Search',msg,SpreadsheetApp.getUi().ButtonSet.OK); }catch(e){}
  return msg;
}
...
function testWeekendMode(){
  ...
  try{ SpreadsheetApp.getUi().alert('Weekend Mode',msg,SpreadsheetApp.getUi().ButtonSet.OK); }catch(e){}
  return msg;
}
...
function checkBudget(){
  ...
  try{ SpreadsheetApp.getUi().alert('Budget',msg,SpreadsheetApp.getUi().ButtonSet.OK); }catch(e){}
  return msg;
}
function testQualityGates(){
  ...
  try{ SpreadsheetApp.getUi().alert('Quality Gates',msg,SpreadsheetApp.getUi().ButtonSet.OK); }catch(e){}
  return msg;
}
function testMonetization(){
  ...
  try{ SpreadsheetApp.getUi().alert('Monetization',msg,SpreadsheetApp.getUi().ButtonSet.OK); }catch(e){}
  return msg;
}
function testBatchDistribute(){
  ...
  try{ SpreadsheetApp.getUi().alert('Batch Distribute',msg,SpreadsheetApp.getUi().ButtonSet.OK); }catch(e){}
  return msg;
}
