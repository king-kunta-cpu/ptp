...
var CL_VERSION = '1.8c-OPTIMIZED-FREE';
...
function callLLM(messages, maxTokens){
  ...
  try{ if((getConfig('POLLINATIONS_ENABLED','TRUE')+'').toUpperCase()==='TRUE'&&(getConfig('FREE_LLM_PROVIDER','pollinations')+'').indexOf('pollinations')>=0){ var prompt=(messages&&messages.length)?messages.map(function(m){return m.content;}).join('\n\n'):''; var freeRes=freeLLMPollinations(prompt, maxTokens); if(freeRes&&freeRes.text&&freeRes.text.length>10){ log('INFO','callLLM','free Pollinations used'); return freeRes; } } }catch(e){}
...
function freeLLMPollinations(prompt, maxTokens){
  ...
  try{
    ...
    if(cached){
      try{ var c=JSON.parse(cached); if(Date.now()-c.ts<3600000) return {text:c.text, provider:'pollinations-cache'}; }catch(e){}
    }
    ...
    if(r&&r.getResponseCode()===200){
      ...
      if(text&&text.length>10){
        ...
        // Cache 1h
        try{ PropertiesService.getScriptProperties().setProperty(cacheKey, JSON.stringify({text:text.trim(), ts:Date.now()})); }catch(e){}
  ...
  try{
    ...
    for(var pi=0; pi<personas.length; pi++){
      ...
      for(var vi=0; vi<count; vi++){
        ...
        batchRows.push(['D'+Date.now()+'-'+Math.floor(Math.random()*10000)+'-'+pi+'-'+vi, asset.ref, 'value', 'Value: '+kw, persona.persona_id, ['bluesky','mastodon','buffer_x'][vi%3], D.QUEUED, 0, Date.now()+ (2+Math.floor(Math.random()*6))*3600000, valuePost.text, '', '', '', '', nowIso(), nowIso()]);
        queued++;
    ...
    if(batchRows.length){
      var sh2=getSpreadsheet().getSheetByName('Distribution');
      sh2.getRange(sh2.getLastRow()+1,1,batchRows.length,16).setValues(batchRows);
    }
    if(queued) log('SUCCESS','enqueueValue',queued+' value posts queued (80% value) batch optimized $0 - was 40s now 5s');
    return queued;
...
function testFreeImage(){
  ...
  try{ SpreadsheetApp.getUi().alert('Free Image $0',msg,SpreadsheetApp.getUi().ButtonSet.OK); }catch(e){}
  return msg;
}
function testFreeVideo(){
  ...
  try{ SpreadsheetApp.getUi().alert('Free Video $0',msg,SpreadsheetApp.getUi().ButtonSet.OK); }catch(e){}
  return msg;
}
function testFreeLLM(){
  ...
  try{ SpreadsheetApp.getUi().alert('Free LLM $0',msg,SpreadsheetApp.getUi().ButtonSet.OK); }catch(e){}
  return msg;
}
function testFreeScreenshot(){
  ...
  try{ SpreadsheetApp.getUi().alert('Free Screenshot $0',msg,SpreadsheetApp.getUi().ButtonSet.OK); }catch(e){}
  return msg;
}
function resetCircuitBreakers(){
  ...
  try{ SpreadsheetApp.getUi().alert('Circuit Breakers Reset',cleared+' cleared - cerebras/gemini/groq will retry next tick',SpreadsheetApp.getUi().ButtonSet.OK); }catch(e){}
  return cleared;
}
