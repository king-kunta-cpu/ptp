...
function callLLM(messages, maxTokens){
  ...
  }catch(e){ log("WARN","emptyCatch","e: "+e.toString()); }
...
function freeLLMPollinations(prompt, maxTokens){
  ...
  try{
    ...
    if(cached){
      try{ var c=JSON.parse(cached); if(Date.now()-c.ts<3600000) return {text:c.text, provider:'pollinations-cache'}; }catch(e){ log("WARN","emptyCatch","e: "+e.toString()); }
    }
    ...
    if(r&&r.getResponseCode()===200){
      ...
      if(text&&text.length>10){
        ...
        // Cache 1h
        try{ PropertiesService.getScriptProperties().setProperty(cacheKey, JSON.stringify({text:text.trim(), ts:Date.now()})); }catch(e){ log("WARN","emptyCatch","e: "+e.toString()); }
...
function gatherAssetScreenshots(ref){
  ...
  if(!out.length){
    ...
    }catch(e2){ log("WARN","emptyCatch","e2: "+e2.toString()); }
  }
  if(!out.length) out.push(freeImagePicsum());
  return out.slice(0,8);
}
...
function clMain(){
  return withLock('clMain', function(){
    ...
    if(isHeavy){
      ...
      try{ if((getConfig('CLOUDFLARE_WORKER_URL','')+'').trim()) { /* Worker does batch distribute */ } }catch(e){ log("WARN","emptyCatch","e: "+e.toString()); }
...
function testFreeImage(){
  ...
  try{ SpreadsheetApp.getUi().alert('Free Image $0',msg,SpreadsheetApp.getUi().ButtonSet.OK); }catch(e){ log("WARN","emptyCatch","e: "+e.toString()); }
  return msg;
}
function testFreeVideo(){
  ...
  try{ SpreadsheetApp.getUi().alert('Free Video $0',msg,SpreadsheetApp.getUi().ButtonSet.OK); }catch(e){ log("WARN","emptyCatch","e: "+e.toString()); }
  return msg;
}
function testFreeLLM(){
  ...
  try{ SpreadsheetApp.getUi().alert('Free LLM $0',msg,SpreadsheetApp.getUi().ButtonSet.OK); }catch(e){ log("WARN","emptyCatch","e: "+e.toString()); }
  return msg;
}
function testFreeScreenshot(){
  ...
  try{ SpreadsheetApp.getUi().alert('Free Screenshot $0',msg,SpreadsheetApp.getUi().ButtonSet.OK); }catch(e){ log("WARN","emptyCatch","e: "+e.toString()); }
  return msg;
}
function resetCircuitBreakers(){
  ...
  try{ SpreadsheetApp.getUi().alert('Circuit Breakers Reset',cleared+' cleared - cerebras/gemini/groq will retry next tick',SpreadsheetApp.getUi().ButtonSet.OK); }catch(e){ log("WARN","emptyCatch","e: "+e.toString()); }
  return cleared;
}
