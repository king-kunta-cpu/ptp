...
function clMain(){
  return withLock('clMain', function(){
    ...
    if(isHeavy){
      ...
      try{ drainDistributionQueue(); }catch(e){ log('ERROR','drain',e.toString()); }
    } else {
      try{
...
function buildPendingAssetsBatch(limit){
  ...
  for(var i=0;i<assets.length&&i<limit;i++){
    ...
    if((wake.verdict==='FAIL'||quality.verdict==='FAIL')&&String(getConfig('ALLOW_FALLBACK_ASSETS','FALSE')).toUpperCase()!=='TRUE'){
      updateRow('Assets',a._row,{status:A.BLOCKED,error:'wake_'+wake.score+' quality_'+quality.score,updated_at:nowIso(),wake_score:wake.score});
      continue;
    }
    updateRow('Assets',a._row,{status:A.BUILT,notion_url:fakeUrl,public_url:fakeUrl,landing_url:'',wake_score:wake.score,updated_at:nowIso()});
    built++;
  }
  if(built) log('SUCCESS','buildBatch',built+' assets built batch quality gated');
  return built;
}
...
function buildPendingAssets(){
  ...
  for(var i=0;i<assets.length;i++){
    ...
    var a=assets[i];
    updateRow('Assets',a._row,{status:A.BUILDING,updated_at:nowIso()});
    // Simulate Notion build - real impl would call Notion API
    var fakeUrl='https://notion.so/'+slugify(a.name)+'-'+a.asset_id;
    var brief=parseJsonSafe(a.spec_json||'')||{};
    var product={name:a.name,price:a.price,deliverable_url:fakeUrl,whop_url:''};
    ...
    var wake=wakeVerifyAsset(product,brief,html);
    if(wake.verdict==='FAIL'&&String(getConfig('ALLOW_FALLBACK_ASSETS','FALSE')).toUpperCase()!=='TRUE'){
      updateRow('Assets',a._row,{status:A.BLOCKED,error:'wake_'+wake.score,updated_at:nowIso(),wake_score:wake.score});
      continue;
    }
    updateRow('Assets',a._row,{status:A.BUILT,notion_url:fakeUrl,public_url:fakeUrl,landing_url:'',wake_score:wake.score,updated_at:nowIso()});
    built++;
  }
  if(built) log('SUCCESS','build',built+' assets built');
  return built;
}
